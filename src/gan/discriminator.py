...
class DiscriminatorSRGAN(nn.Module):
    def __init__(self, in_channels=1, channel_list=[32, 64, 128, 256, 512], lr=1e-6):
        ...
        # Input layer
        layers.extend([
            nn.Conv2d(current_channels, channel_list[0], kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2, inplace=True)
        ])
        ...
        for i in range(len(channel_list)-1):
            ...
            layers.extend([
                ...
                # Second conv maintaining spatial dimensions
                nn.Conv2d(channel_list[i], channel_list[i+1], kernel_size=3, stride=1, padding=1),
                nn.LeakyReLU(0.2, inplace=True)
            ])
